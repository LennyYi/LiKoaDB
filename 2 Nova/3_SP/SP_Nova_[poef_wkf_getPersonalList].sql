if exists (select 1 from dbo.sysobjects where id = object_id(N'[dbo].[poef_wkf_getPersonalList]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	drop procedure [dbo].[poef_wkf_getPersonalList]
GO
/*********************** REVISION LOG: ***********************************
Version			Date			Revised by				Remark
1.0				20150304		Sting Wu				initial
**************************************************************************/
CREATE PROCEDURE [dbo].[poef_wkf_getPersonalList]
...
select REQUESTNO = t1.request_no,  
	...
	VIEWFLAG = (CASE WHEN (t1.request_staff_code = @staff_code OR t1.submit_staff_code = @staff_code) THEN '0' ELSE '1' END),
	CANDELETE = (CASE WHEN (t1.request_staff_code = @staff_code OR t1.submit_staff_code = @staff_code)
				AND (t1.[status] = '00' OR (t1.node_id = '0' and t1.[status] = '03')) THEN '1' ELSE '0' END)
from #tempTable2 t1
