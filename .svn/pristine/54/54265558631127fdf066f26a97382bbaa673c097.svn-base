IF EXISTS (SELECT * FROM SYSOBJECTS WHERE ID = OBJECT_ID('dbo.USPGNOVAOFFICIALBILL_TM') AND sysstat & 0xf = 4)
	drop procedure dbo.USPGNOVAOFFICIALBILL_TM
GO
...
SET ANSI_PADDING ON
go
CREATE PROCEDURE dbo.USPGNOVAOFFICIALBILL_TM
...
AND A.POLNO = C.POLNO AND A.PRODCODE = C.NOVA_PRODCODE AND B.Prodcode = C.SUBPRODCODE
