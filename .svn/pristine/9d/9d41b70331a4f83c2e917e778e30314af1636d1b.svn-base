...
BEGIN
	...
	(NOVAREQNO,POLNO,BILLPRDFR,PRODCODE,PRODSHORT,[STATE],PREMPAID,PREMANNGROW)
	SELECT NOVAREQNO,POLNO,BILLPRDFR,A.PRODCODE,PRODSHORT,[STATE],SUM(PaidPrem),SUM(CASE A.CHGSTATUS WHEN 'ADD' THEN PaidPrem ELSE 0 END) FROM TNOVA_SDMemPremDtl A, TPRODUCT B
...
BEGIN
	INSERT #TNOVA_SDPremSum
	(NOVAREQNO,POLNO,BILLPRDFR,PRODCODE,PRODSHORT,[STATE],PREMPAID,PREMANNGROW)
	SELECT NOVAREQNO,POLNO,BILLPRDFR,A.PRODCODE,PRODSHORT,[STATE],SUM(PaidPrem),SUM(CASE WHEN INIEFFDT >= @dASSIGNDATE THEN PaidPrem ELSE 0 END) FROM TNOVA_SDMemPremDtl A, TPRODUCT B
	WHERE A.NOVAREQNO = @cNovaReqno
	AND B.PRODCODE = A.PRODCODE
	GROUP BY NOVAREQNO,POLNO,BILLPRDFR,A.PRODCODE,PRODSHORT,[STATE]
END
